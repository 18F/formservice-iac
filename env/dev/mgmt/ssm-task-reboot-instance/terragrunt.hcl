...
inputs = {
  ...
  commands                  = [
    "export ID=$(curl -s http://169.254.169.254/latest/meta-data/instance-id) && export ENV=$(curl -s http://169.254.169.254/latest/meta-data/public-keys | sed \"s/-.*//; s/.*=//\") && if [[ \"$(sudo service docker status)\" == *\"dead\"* ]] ; then aws sns publish --topic-arn arn:aws-us-gov:sns:us-gov-west-1:${local.account_num}:Forms-Service-Issue-Alert --message 'The docker daemon is dead on ec2 instance $ID in the $ENV environment. AWS Systems Manager is rebooting the instance now. This message and reboot command was triggered by an AWS Systems Manager maintenance window task.' && sudo reboot ; fi"
  ]
