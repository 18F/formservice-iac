...
terraform {
  source = "git@github.com-gsa:18F/formservice-iac-modules.git//elastic-beanstalk-sub"
}
...
inputs = {
  name_prefix = "${local.name_prefix}-epa"
  ...
  version_name = "faas-prod-runtime-submission-app-v9.0.32-cis-5.12-cis-5.25"
  ...
  MONGO              = "mongodb://${get_env("doc_db_user")}:${get_env("doc_db_user_password")}@${dependency.documentdb.outputs.endpoint}:27017/${get_env("doc_db_database")}?ssl=true"
  PORTAL_BASE_URL    = "https://portal.forms.gov"
